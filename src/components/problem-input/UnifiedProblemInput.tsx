...
interface UnifiedProblemInputProps {
  embedded?: boolean; // When true, renders without hero section for embedding in other layouts
}

export function UnifiedProblemInput({ embedded = false }: UnifiedProblemInputProps = {}) {
  ...
  // Content that appears in both embedded and standalone modes
  const inputContent = (
    <>
      {/* Primary Action: Solve a Problem */}
      <div className="mb-6">
        <div className={embedded
          ? "bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border-2 border-blue-400 overflow-hidden"
          : "bg-white rounded-2xl shadow-lg border-2 border-blue-500 overflow-hidden"
        }>
      ...
      <div className={embedded
        ? "bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 overflow-hidden"
  ...
  // Return embedded version (just the input content)
  if (embedded) {
    return <div className="w-full">{inputContent}</div>;
  }

  // Return standalone version (with hero section and full page layout)
  return (
